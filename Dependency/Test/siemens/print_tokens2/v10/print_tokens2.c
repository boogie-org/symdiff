...
{ 
  ...
  int id=0;
  char ch,ch1[2];
  for (j=0;j<=const_80;j++)          /* initial the buffer   */
      { buffer[j]='\0';} 
   ch1[0]='\0';
   ch1[1]='\0';
   ch=get_char(tp);
...
{ int typea;
 ...
 if(typea==spec_symbol)print_spec_symbol(tok);
 if(typea==identifier)
   {fprintf(stdout, "identifier,\"%s\".\n",tok);
   }
 if(typea==num_constant)
   {fprintf(stdout, "numeric,%s.\n",tok);
   }
 if(typea==str_constant)
   {fprintf(stdout, "string,%s.\n",tok);
   }
 if(typea==char_constant)
   {tok=tok+1;
